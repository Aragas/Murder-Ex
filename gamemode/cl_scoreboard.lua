...
function GM:DoScoreboardActionPopup(ply)
	...
	if ply:IsAdmin() then
		local admin = actions:AddOption(Translator:Client(LocalPlayer()).scoreboardActionAdmin)
		admin:SetIcon("icon16/shield.png")
	end

	if ply != LocalPlayer() then
		local t = Translator:Client(LocalPlayer()).scoreboardActionMute
		if ply:IsMuted() then
			t = Translator:Client(LocalPlayer()).scoreboardActionUnmute
		end
		...
		local steamId = actions:AddOption(Translator:Client(LocalPlayer()).scoreboardGetSteamId)
	...
	if IsValid(LocalPlayer()) && LocalPlayer():IsAdmin() then
		...
		if ply:Team() == 2 then
			local spectate = actions:AddOption(Translator:QuickVar(Translator:Client(LocalPlayer()).adminMoveToSpectate, "spectate", Translator:Client(LocalPlayer()).teamSpectators))
			spectate:SetIcon("icon16/status_busy.png")
			function spectate:DoClick()
				RunConsoleCommand("mu_movetospectate", ply:EntIndex())
			end

			local force = actions:AddOption(Translator:Client(LocalPlayer()).adminMurdererForce)
			...
			if ply:Alive() then
				local specateThem = actions:AddOption(Translator:Client(LocalPlayer()).adminSpectate)
...
local function makeTeamList(parent, pteam)
	local teamName
	if(pteam == 1) then
		teamName = Translator:Client(LocalPlayer()).teamSpectators
	else
		teamName = Translator:Client(LocalPlayer()).teamPlayers
	end
	...
	but:Dock(RIGHT)
	but:SetText(Translator:Client(LocalPlayer()).scoreboardJoinTeam)
...
function GM:ScoreboardShow()
	if IsValid(menu) and menu.Lang and menu.Lang == Translator:ClientLang(LocalPlayer()) then
		menu:SetVisible(true)
	else
		menu = vgui.Create("DFrame")
		menu.Lang = Translator:ClientLang(LocalPlayer())
		...
		menu.Watchers = makeTeamList(menu, 1)
		menu.Watchers:Dock(FILL)
	end
end
