...
local function addPlayerItem(self, mlist, ply, pteam)
	...
	function but:Paint(w, h)
		...
		if IsValid(ply) && ply:IsPlayer() then
			...
			local status = Translator:Client(LocalPlayer()).bystander
			local statusColor = team.GetColor(2)
			if !ply:Alive() then
				status = Translator:Client(LocalPlayer()).playerStatusDead
			...
			elseif playerData && playerData.players[ply:EntIndex()] && playerData.players[ply:EntIndex()].murderer then
				status = Translator:Client(LocalPlayer()).murderer
...
local function makeTeamList(parent, pteam)
	...
	function headp:Paint(w, h)
		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardPing, "ScoreboardPlayer", w - 9, 2, color_black, 2)
		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardPing, "ScoreboardPlayer", w - 10, 2, color_white, 2)

		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardBystanderName, "ScoreboardPlayer", w * 0.4 + 1, 2, color_black, 0)
		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardBystanderName, "ScoreboardPlayer", w * 0.4, 2, color_white, 0)

		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardStatus, "ScoreboardPlayer", w * 0.64 + 1, 2, color_black, 0)
		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardStatus, "ScoreboardPlayer", w * 0.64, 2, color_white, 0)

		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardChance, "ScoreboardPlayer", w * 0.86 + 1, 2, color_black, 0)
		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardChance, "ScoreboardPlayer", w * 0.86, 2, color_white, 0)

		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardName, "ScoreboardPlayer", 11, 2, color_black, 0)
		draw.DrawText(Translator:Client(LocalPlayer()).scoreboardName, "ScoreboardPlayer", 10, 2, color_white, 0)
	end
...
concommand.Add("mu_adminpanel", function (client)
	...
	if IsValid(menu) then
		...
		menu:SetTitle(Translator:Client(LocalPlayer()).adminPanel)
		...
		refresh:Dock(TOP)
		refresh:SetText(Translator:Client(LocalPlayer()).scoreboardRefresh)
